...
function BuscaIgrejas() {
  ...
  const [allParoquias, setAllParoquias] = useState([]);

  useEffect(() => {
    fetch('/api/paroquias')
      .then(res => res.json())
      .then(data => setAllParoquias(Array.isArray(data) ? data : []))
  ...
  const igrejasFiltradas = buscaTrim
    ? allParoquias.filter((igreja) => {
        ...
        const horarioCombina = Array.isArray(igreja.horarios) && igreja.horarios.some((h) => {
          const texto = typeof h === 'object' ? `${h.diaSemana} ${h.hora} ${h.tipo}` : String(h);
