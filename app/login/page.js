...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 401) setError('Palavra-passe incorreta.');
        else if (err.response.status === 404) setError('Utilizador não encontrado.');
        else setError(err.response.data?.error || 'Erro ao tentar fazer login.');
      } else {
  ...
  return (
    <div className="login-container">
      ...
      <form onSubmit={handleSubmit} className="login-form" style={{ maxWidth: 400, margin: '0 auto', background: '#fff', borderRadius: '16px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', padding: '32px 18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      <p>Ainda não tem conta? <Link href="/register">Registe-se aqui</Link></p>
    </div>
  );
};
